...
import { BASE_Z } from './stageConfig';
...
/**
 * Altura visivel do quadro, em unidades de mundo, na profundidade `z`.
 *
 * Sem esta conversao a mao seria dimensionada na escala do plano z=0 e, posta
 * em primeiro plano, apareceria varias vezes maior que o quadro — visivel
 * apenas como uma mancha vermelha na borda.
 */
const visibleHeightAt = (z: number): number => (HEIGHT * (BASE_Z - z)) / BASE_Z;

/**
 * A MAO VERMELHA — gesto, mascara e transicao.
 *
 ...
 */
export const RedHandTransition: React.FC<Props> = ({
  frame,
  window: [start, end],
  from = 'right',
  coverage = 1.55,
  ...
  const geometry = useMemo(() => {
    ...
    const h = visibleHeightAt(z) * coverage;
  ...
  const travel = ((WIDTH * (BASE_Z - z)) / BASE_Z) * 1.35;
  ...
  return (
    <group position={[x, visibleHeightAt(z) * 0.03, z]} rotation={[0, 0, rad(tilt)]}>
      {ghostMaterial
